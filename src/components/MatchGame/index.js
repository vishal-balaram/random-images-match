...
class MatchGame extends Component {
  ...
  setInitialState = () => {
    ...
    this.setState({timeElapsedInSeconds: 60})
    this.setState({score: 0})
    this.setState({imageUrl: imagesList[5].imageUrl})
  }

  filterTabItems = () => {
    const {activeTabId} = this.state
    const filteredTabsList = imagesList.filter(
  ...
  randomImageUrl = () => {
    ...
    this.setState({imageUrl: imagesList[randomNumber].imageUrl})
  }

  updateScoreIfCorrect = imageUrll => {
    const {imageUrl} = this.state
    if (imageUrl === imageUrll) {
  ...
  render() {
    const {activeTabId, score, timeElapsedInSeconds, imageUrl} = this.state
    const filteredTabItems = this.filterTabItems()
    const checkTimer = timeElapsedInSeconds < 0
    ...
    return (
      <div>
        <nav className="navbar">
          <div className="top-navbar-container">
            ...
            <button
              ...
              onClick={this.startSecDecreament}
              type="button"
            >
              Start
            </button>
            <div className="score-alba-container">
              <p className="score">
                Score: <span className="span-0">{score}</span>
              </p>
              <img
              ...
              {timeElapsedInSeconds > 0 ? (
                <p className="alba">{timeElapsedInSeconds} secs</p>
              ) : (
                <p className="alba">0 secs</p>
              )}
            </div>
          </div>
        </nav>
        <div className="bottom-bg-container">
          {!checkTimer ? (
            <div className="hi">
              <img src={imageUrl} alt="match" className="top-main-image" />
              <div>
                ...
                <ul className="unordered-list">
                  {filteredTabItems.map(EachImage => (
                    <ThumbNailItem
                      ...
                      key={EachImage.id}
                      updateScoreIfCorrect={this.updateScoreIfCorrect}
                    />
                  ))}
                </ul>
                <ul>
            ...
            <div className="game-over-container">
              <div className="game-over-image">
                ...
                <h1 className="your-score-para">{score}</h1>
                <button
